...
app.post("/reviews", async (req, res) => {
    ...
    /* if (a.indexOf('udemy') > -1) {
        req.body.platformName = 'udemy';
        var d1 = a.replace('-', '').split("/");
        var l = d1.length;
        req.body.courseName = d1[l - 1];
    } else {
        const metadata = await urlMetadata(req.body.courseURL);
        // console.log(metadata['favicons'][0].href);
        // console.log(metadata['og:title']);
        req.body.courseName = metadata['og:title'];
        var l = metadata['favicons'].length;
        if (l == 0) {
            req.body.Logo = metadata['og:image'];
        } else {
            req.body.Logo = metadata['favicons'][l - 1].href;
        }
        ...
        if ((img.indexOf("http://") < 0) && (img.indexOf("https://") < 0)) {
            const d1 = d.split("/");
            req.body.Logo = 'https://'.concat(d1[0].concat(req.body.Logo));
        }
